...
def gerar_pix_misticpay(valor: float, telegram_id: int):
    ...
    payload = {
        ...
        # Adicione aqui outros campos exigidos pela documentação da MisticPay
    }
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_principal":
    ...
    elif data == "catalogo_unitarias":
        texto = "📦 *Catálogo de Unitárias:*\n\n"
        for key, item in CATALOGO_UNITARIAS.items():
            texto += f"• *{item['nome']}*: {item['preco']} | Estoque: `{item['estoque']}`\n"
        
        keyboard = [[InlineKeyboardButton("⬅️ Voltar", callback_data="menu_comprar")]]
        await query.message.reply_text(texto, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

    elif data == "catalogo_bins":
        bins_summary = {}
        for item in DADOS_PLATINUM:
            b = item["bin"]
            if b not in bins_summary:
                bins_summary[b] = {"banco": item["banco"], "qtd": 0, "preco": item["preco"]}
            bins_summary[b]["qtd"] += 1

        texto = "💳 *Estoque Disponível por BIN:*\n\n"
        for b, info in bins_summary.items():
            texto += f"🔹 *BIN:* `{b}` | {info['banco']}\n  └ Qtd: `{info['qtd']}` | Valor: R$ {info['preco']}\n\n"
        ...
        texto = (
            "💳 *Adicionar Saldo*\n\n"
            "Digite o valor que deseja recarregar via Pix utilizando o comando:\n"
            "`/recarga VALOR` (Exemplo: `/recarga 50`)"
        )
        ...
        keyboard = [[InlineKeyboardButton("⬅️ Voltar", callback_data="menu_comprar")]]
        await query.message.reply_text(POLITICA_REEMBOLSO, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

    elif data in ["info", "ferramentas", "indicacoes"]:
        texto = f"ℹ️ *Sessão {data.capitalize()}*\nEm breve novas atualizações aqui!"
        keyboard = [[InlineKeyboardButton("⬅️ Voltar", callback_data="menu_principal")]]
        await query.message.reply_text(texto, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

async def comando_recarga(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    try:
        valor = float(context.args[0])
        pix_data = gerar_pix_misticpay(valor, user_id)
        if pix_data and "qrcode" in pix_data:
            await update.message.reply_text(
                f"✅ *Cobrança Gerada!*\n\nCopie o código Pix abaixo:\n\n`{pix_data['qrcode']}`",
                parse_mode="Markdown"
            )
        else:
            await update.message.reply_text("❌ Falha ao gerar Pix com a MisticPay. Tente novamente mais tarde.")
    except (IndexError, ValueError):
        await update.message.reply_text("⚠️ Use o formato correto: `/recarga 50`", parse_mode="Markdown")

# Registrar Handlers
telegram_app.add_handler(CommandHandler("start", start))
telegram_app.add_handler(CommandHandler("recarga", comando_recarga))
telegram_app.add_handler(CallbackQueryHandler(callback_handler))

# ==================== FASTAPI WEBHOOK LIFESPAN ====================
@asynccontextmanager
async def lifespan(app: FastAPI):
    # Setup Lifespan do Telegram Bot
    ...
    logger.info("Bot do Telegram desligado.")
...
async def misticpay_webhook(request: Request):
    ...
    # Trate aqui a liberação de saldo do usuário ao confirmar o pagamento
    return {"status": "success"}

@app.get("/")
async def index():
    return {"message": "CasaBlanca Bot rodando normalmente."}
